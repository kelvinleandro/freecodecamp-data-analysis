...
def calculate_demographic_data(print_data=True):
    ...
    rich_countries = (df[df['salary'] == '>50K']['native-country'].value_counts() / df['native-country'].value_counts() * 100).sort_values(ascending=False)
    highest_earning_country = rich_countries.index[0]
    highest_earning_country_percentage = round(rich_countries[0], 1)

    # Identify the most popular occupation for those who earn >50K in India.
    india = df[(df['native-country'] == 'India') & (df['salary'] == '>50K')]['occupation'].value_counts()
